...
export function MainHeader() {
  ...
  const items = links.map((link) => (
    <a
      key={link.label}
      href={link.link}
      className="px-3 py-2 hover:bg-gray-200 active:bg-gray-200 text-lg flex items-center gap-2 rounded text-white hover:text-black active:text-black transition-colors"
    >
      {link.label === "Scout" && (
